...
def enabled_head():
    return html.Div([
        html.Div('Разрешить‌', style=head_style, className='me-2 align-bottom', id='tt-enabled'),
        dbc.Tooltip('Разрешить движение каретки', target='tt-enabled', placement='top'),
        html.Div([
            dbc.Button('✓', id='enabled-set-all', size='sm', style=head_style, class_name='align-bottom'),
            dbc.Tooltip('Выбрать все', target='enabled-set-all', placement='bottom'),
            dbc.Button('✗', id='enabled-reset-all', size='sm', class_name='align-bottom'),
            dbc.Tooltip('Сбросить все', target='enabled-reset-all', placement='bottom')
        ], className='d-block')
    ])


def enabled_column(pd_table):
    return pd_table[['idx', 'enabled']].apply(
        lambda x: make_checkbox({'type': 'enabled', 'index': str(x['idx'])}, x['enabled']), axis=1)
...
def carmove2_head(carmove2):
    return html.Div([
        html.Div('Параметры 1', style=head_style, className='me-2 align-bottom', id='tt-carmove2'),
...
def make_carriage_html_table(pd_table):
    table_out_df = pd.DataFrame({
        ...
        enabled_head(): enabled_column(pd_table),
